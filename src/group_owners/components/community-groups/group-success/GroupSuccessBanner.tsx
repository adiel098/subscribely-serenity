...
export const GroupSuccessBanner: React.FC<GroupSuccessBannerProps> = ({
  ...
  const copyGroupMiniAppLink = () => {
    if (!groupId) return;
    
    const miniAppUrl = customLink 
      ? `https://t.me/${botUsername}?start=${customLink}`
      : `https://t.me/${botUsername}?start=${groupId.replace('group_', '')}`;
    ...
    navigator.clipboard.writeText(miniAppUrl)
      .then(() => {
        ...
        toast({
          ...
          description: "Your Group Mini App link is ready to share! 🚀",
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-emerald-800">Your group is ready! 🎉</span>
      </div>
      ...
      <div className="flex items-center gap-2 ml-2 bg-white/70 rounded-md px-2 py-1 border border-emerald-100 min-w-[300px] max-w-[400px]">
        ...
        <span className="text-xs text-emerald-700 font-mono truncate">
          t.me/{botUsername}?start={customLink || groupId?.substring(0, 8) + "..."}
        </span>
      </div>
