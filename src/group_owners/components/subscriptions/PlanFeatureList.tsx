    ...
    <div className="space-y-2">
      ...
      <ul className="space-y-2">
        {features.map((feature, index) => (
          <motion.li 
            key={index} 
            className="flex items-center justify-between gap-1.5 py-1.5 px-2.5 bg-gradient-to-r from-indigo-50 to-indigo-100/60 rounded-md text-sm group transition-all hover:from-indigo-100/80 hover:to-indigo-200/70 border border-indigo-100/50"
            initial={{ opacity: 0, y: 5 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.2, delay: index * 0.05 }}
          >
            <div className="flex items-center gap-2">
              <div className="h-5 w-5 rounded-full bg-white flex items-center justify-center flex-shrink-0 shadow-sm">
