    ...
    <div id="payment-methods" className="space-y-8 animate-fade-in pb-12">
      <div className="text-center space-y-2">
        <Badge variant="secondary" className="px-4 py-1.5">
          ...
          {showSuccess ? "Join Now!" : "Final Step"}
        </Badge>
        <h2 className="text-3xl font-bold text-gray-900">
          {showSuccess ? "Ready to Join" : "Choose Payment Method"}
        </h2>
      ...
      {showSuccess && communityInviteLink && (
        <div className="flex flex-col items-center space-y-4 animate-fade-in">
          <Button 
            size="lg" 
            className="px-8 py-6 text-lg font-semibold gap-2 w-full max-w-sm"
            onClick={() => window.location.href = communityInviteLink}
          >
            <ArrowRight className="h-5 w-5" />
            Join Community
          </Button>
          <p className="text-sm text-muted-foreground pb-8">
            Click the button above to join the Telegram community
          </p>
        </div>
      )}
    </div>
